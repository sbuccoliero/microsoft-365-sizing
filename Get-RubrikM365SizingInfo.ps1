...
# Validate the required 'RobustCloudCommand' is installed
# and provide a user friendly message when it's not.
if (Get-Module -ListAvailable -Name RobustCloudCommand)
{
    
}
else
{
    throw "The 'RobustCloudCommand' is required for this script. Run the follow command to install: Install-Module RobustCloudCommand"
}
...
try {
    ...
    Write-Output "Achivemailboxes count: $ArchiveMailboxesCount" 
    ...
    # Remove the RobustCloudCommand log file if it exists
...
try {
    ...
    if ($SharedMailboxesCount -le $LargeAmountofSharedMailboxCount) {
        ...
        $SharedMailboxesSize +=  RobustCloudCommand -UserPrincipalName $ManualUserPrincipalName -IdentifyingProperty "DisplayName" -recipients $SharedMailboxeFirstInterval -logfile "$systemTempFolder\sharedMailbox.log" -ScriptBlock {Get-ExoMailboxStatistics -Identity $input.UserPrincipalName | Select-Object TotalItemSize}
        Write-Output ""
    }
